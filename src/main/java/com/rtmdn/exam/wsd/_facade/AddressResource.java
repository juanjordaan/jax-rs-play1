
package com.rtmdn.exam.wsd._facade;

import javax.ejb.EJB;
import javax.ejb.Stateless;
import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;
import javax.ws.rs.core.Response;

import com.rtmdn.exam.wsd._model.employee.Address;
import com.rtmdn.exam.wsd._service.AddressService;

@Path( "address" )
@Stateless
public class AddressResource extends ProjectApp
{
	@EJB
	private AddressService addressService;
	
	@POST
	@Consumes( MediaType.APPLICATION_JSON )
	@Produces( MediaType.APPLICATION_JSON )
	public Response createAddress( Address address )
	{
		address = addressService.createAddress ( address );
		
		return Response.ok (  ).entity ( address ).build ( );
	}
	
	@GET
	@Produces( MediaType.APPLICATION_JSON )
	public Response list( )
	{
		return Response.ok ( ).entity ( addressService.getAddresses ( ) ).build ( );
	}
	
	@GET
	@Path("/{id}")
	@Produces( MediaType.APPLICATION_JSON )
	public Response getAddress( @PathParam(value="id") Long id )
	{
		return Response.ok ( ).entity ( addressService.getAddresses ( ) ).build ( );
	}
	
	@PUT
	@Consumes( MediaType.APPLICATION_JSON )
	@Produces( MediaType.APPLICATION_JSON )
	public Response updateAddress( Address address )
	{
		Address copy = addressService.updateAddress ( address );
		
		if( address.getVersion().equals( copy.getVersion( ) ) )
		{
			return Response.notModified ( ).build ( );
		}
		
		return Response.ok ( ).entity ( copy ).build ( );
	}
	
	@DELETE
	@Consumes( MediaType.APPLICATION_JSON )
	@Produces( MediaType.APPLICATION_JSON )
	public Response deleteAddress( Address address )
	{
		addressService.deleteAddress ( address );
		
		return Response.noContent ( ).build ( );
	}
}
