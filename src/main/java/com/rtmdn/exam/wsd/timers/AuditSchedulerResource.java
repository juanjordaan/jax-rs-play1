...
public class AuditSchedulerResource
{
	...
	{
		return new ScheduleExpression ( ).second ( config.getSecond ( ) ).minute ( config.getMinute ( ) ).hour ( config.getHour ( ) ).dayOfWeek ( config.getDayOfWeek ( ) )
			.dayOfMonth ( config.getDayOfMonth ( ) ).month ( config.getMonth ( ) ).year ( config.getYear ( ) );
	}
}
