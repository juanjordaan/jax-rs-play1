...
public class ScheduledAuditService
{
	...
	{
		boolean timerExists = tsvc.getTimers ( ).stream ( ).anyMatch ( ( timer ) -> ((String) timer.getInfo ( )).equals ( timerConfig.getInfo ( ) ) );
	...
	{
		boolean timerExists = tsvc.getTimers ( ).stream ( ).anyMatch ( ( timer ) -> ((String) timer.getInfo ( )).equals ( name ) );
		...
		return tsvc.getTimers ( ).stream ( ).filter ( ( timer ) -> ((String) timer.getInfo ( )).equals ( name ) )
			.map ( ( timer ) -> new ScheduledTimerInfo ( name, timer.getSchedule ( ), timer.getTimeRemaining ( ), timer.getNextTimeout ( ).toString ( ) ) ).findFirst ( ).get ( );
	...
	public void cancel( String name )
	{
		...
		tsvc.getTimers ( ).stream ( ).filter ( ( timer ) -> ((String) timer.getInfo ( )).equals ( name ) ).findFirst ( ).get ( ).cancel ( );
	}
